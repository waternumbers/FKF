test_that("FKS matches simple implimentation", {
    ...
    mt <- fkf.obj$att
    for(ii in n:1){
        ## apply - recall at start Vt[[,ii] is Ptt[,,ii] and mt[,ii] is filtered estimate
        mt[,ii] <- mt[,ii] + Vt[,,ii]%*%r
        ...
        Z <- fkf.obj$Zt[,,1]; Z <- Z[idx,,drop=FALSE]
        L <- as.matrix(fkf.obj$Tt[,,1]) - K%*%Z #fkf.obj$Kt[,idx,ii]%*%fkf.obj$Zt[idx,,1]
        r <- t(Z) %*% fkf.obj$Ftinv[idx,idx,ii] %*% matrix(fkf.obj$vt[idx,ii]) + t(L)%*%r
        N <- t(Z)%*% fkf.obj$Ftinv[idx,idx,ii] %*% Z + t(L)%*%N%*%L
    ...
    testthat::expect_equal( Vt, fks.obj$Vt )
